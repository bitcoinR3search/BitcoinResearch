...
ax.patch.set_facecolor(Estilos[tipo][0])
...
ax.tick_params(axis='both',colors=Estilos[tipo][1])
...
ax.plot(fecha_array[360*6:-2],precio_n[360*6:-2],color=colores[3],label='Precio BTC')
ax.plot(fecha_array[360*6:-2],hash_n[360*6:-2],color=colores[5],label='*Hash-Rate')
...
ax.text(x_value,.8, '*Media Móvil\nSobre 2016 Bloques',color=Estilos[tipo][1], ha='right', va='center', size=15)
...
for i in indices_cruce[-4:]:
    ...
    ax.scatter(fecha_array[360*6:][i+1], precio_n[360*6:][i+1], color='black',s=30) 
...
x_value = mdates.date2num(date)
ax.vlines(x_value, 0,.5, colors=Estilos[tipo][1], linestyles='dashed')
date = datetime(2016,5,9)
x_value = mdates.date2num(date)
ax.text(x_value, .5, '2do\nHalv',color=Estilos[tipo][1], ha='right', va='center', size=13)
...
plt.savefig('analisis/resultados/pricHash_'+tipo +'.png', bbox_inches='tight', pad_inches=0.75)